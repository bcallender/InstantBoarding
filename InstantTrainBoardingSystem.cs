...
{
    public partial class InstantBoardingTrainSystem : GameSystemBase
    {
        ...
        public InstantBoardingTrainSystem()
        {
        }
        ...
        private struct TransportTrainTickJob : IJobChunk
        {
            ...
            public void Execute(in ArchetypeChunk chunk, int unfilteredChunkIndex, bool useEnabledMask, in v128 chunkEnabledMask)
            {
                ...
                for (int i = 0; i < nativeArray.Length; i++)
                {
                    ...
                    Tick(chunk, unfilteredChunkIndex, ref random, vehicleEntity, owner, prefabRef, currentRoute, layout, navigationLanes, serviceDispatches, isUnspawned, ref cargoTransport, ref publicTransport, ref pathOwner, ref target, ref odometer);
            ...
            private void Tick(in ArchetypeChunk chunk, int jobIndex, ref Random random, Entity vehicleEntity, Owner owner, PrefabRef prefabRef, CurrentRoute currentRoute, DynamicBuffer<LayoutElement> layout, DynamicBuffer<TrainNavigationLane> navigationLanes, DynamicBuffer<ServiceDispatch> serviceDispatches, bool isUnspawned, ref Game.Vehicles.CargoTransport cargoTransport, ref Game.Vehicles.PublicTransport publicTransport, ref PathOwner pathOwner, ref Target target, ref Odometer odometer)
            {
                ...
                if (!m_PrefabRefData.HasComponent(target.m_Target) || VehicleUtils.PathfindFailed(pathOwner))
                {
                    ...
                    {
                        flag = true;
                        StopBoarding(chunk, jobIndex, ref random, vehicleEntity, currentRoute, layout, ref cargoTransport, ref publicTransport, ref target, ref odometer, isCargoVehicle, forcedStop: true);
                    }
                ...
                else if (VehicleUtils.PathEndReached(currentLane))
                {
                    if ((cargoTransport.m_State & (CargoTransportFlags.Returning | CargoTransportFlags.DummyTraffic)) != 0 || (publicTransport.m_State & (PublicTransportFlags.Returning | PublicTransportFlags.DummyTraffic)) != 0)
                    {
                        if ((cargoTransport.m_State & CargoTransportFlags.Boarding) != 0 || (publicTransport.m_State & PublicTransportFlags.Boarding) != 0)
                        {
                            if (StopBoarding(chunk, jobIndex, ref random, vehicleEntity, currentRoute, layout, ref cargoTransport, ref publicTransport, ref target, ref odometer, isCargoVehicle, forcedStop: false))
                            {
                                flag = true;
            ...
            private void FindPathIfNeeded(Entity vehicleEntity, PrefabRef prefabRef, Entity skipWaypoint, ref TrainCurrentLane currentLane, ref Game.Vehicles.CargoTransport cargoTransport, ref Game.Vehicles.PublicTransport publicTransport, ref PathOwner pathOwner, ref Target target)
            {
                if (VehicleUtils.RequireNewPath(pathOwner))
                {
                    ...
                    pathfindParameters.m_IgnoredRules = RuleFlags.ForbidCombustionEngines | RuleFlags.ForbidHeavyTraffic | RuleFlags.ForbidPrivateTraffic | RuleFlags.ForbidSlowTraffic;
            ...
            private bool StopBoarding(in ArchetypeChunk chunk, int jobIndex, ref Random random, Entity vehicleEntity, CurrentRoute currentRoute, DynamicBuffer<LayoutElement> layout, ref Game.Vehicles.CargoTransport cargoTransport, ref Game.Vehicles.PublicTransport publicTransport, ref Target target, ref Odometer odometer, bool isCargoVehicle, bool forcedStop)
            {
                bool flag = false;
                ...
                if (!forcedStop)
                {
                    ...
                    if (layout.Length != 0)
                    {
                        for (int i = 0; i < layout.Length; i++)
                        {
                            if (!ArePassengersReady(jobIndex, layout[i].m_Vehicle))
                            {
                                return false;
                            }
                        }
                    }
                    else if (!ArePassengersReady(jobIndex, vehicleEntity))
                    {
                        return false;
                    }
                }
            ...
            /// <summary>
            /// This is the main function that has been modified
            /// </summary>
            private bool ArePassengersReady(int jobIndex, Entity vehicleEntity)
            {
                ...
                for (int i = 0; i < dynamicBuffer.Length; i++)
                {
                    ...
                    if ((m_CurrentVehicleData[passenger].m_Flags & CreatureVehicleFlags.Entering) != 0)
                    {
                        if(m_CurrentVehicleData.TryGetComponent(passenger, out var currentVehicleData))
                        {
                            currentVehicleData.m_Flags |= CreatureVehicleFlags.Ready;
                            currentVehicleData.m_Flags &= ~CreatureVehicleFlags.Entering;
                            m_CommandBuffer.SetComponent(jobIndex, passenger, currentVehicleData);
                            m_CommandBuffer.AddComponent(jobIndex, passenger, default(BatchesUpdated));
